...
The map below shows the number of days containing observations in each 1° grid cell, taken from the SOCATv2021 dataset.
	...
	for lon in 1:360
		for lat in 1:180
			if !ismissing(timeseriesinfonc["minute_R̅"][lon,lat])
				push!(timeseriesinfo, [lon - 0.5, lat, timeseriesinfonc["count"][lon,lat], timeseriesinfonc["minute_R̅"][lon,lat]])
			end
		end
	end
	
	histogram(timeseriesinfo[!, "R̅"], label="", bins=50, title="Distribution of R̅", xlabel="R̅", ylabel="Count")
end
...
scatter(timeseriesinfo[!, "count"], timeseriesinfo[!, "R̅"], label="", xlabel="Time Series Count (log₁₀)", xlims=(500, maximum(timeseriesinfo[!, "count"])), ylabel="R̅", xscale=:log10, markerstrokewidth=0, title="R̅ for cells with time series count ≥ 500")
